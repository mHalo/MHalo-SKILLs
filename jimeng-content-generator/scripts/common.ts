...
/**
 * 获取默认输出目录
 * 优先使用 OPENCLAW_MEDIA_PATH 环境变量，图片/视频将保存到该目录下的 jimeng-generate 文件夹
 * 如果未设置，则使用默认的 ./output
 */
export function getDefaultOutputDir(): string {
  const fs = require('fs');
  const path = require('path');
  const openclawPath = process.env.OPENCLAW_MEDIA_PATH;
  
  if (openclawPath) {
    // 确目录存在
    const fullPath = path.join(openclawPath, 'jimeng-generate');
