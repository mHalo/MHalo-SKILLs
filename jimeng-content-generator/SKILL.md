...
description: 基于火山引擎即梦AI的文生图/文生视频能力，支持通过文本描述生成图片和视频。必须配置 OpenClaw media 目录，图片将直接保存到该目录供 OpenClaw 读取展示。
---
...
```bash
# OpenClaw Media 路径 - 生成的图片将保存到该目录下的 jimeng-generate 文件夹
...
初始开始，如果Key不存在则说明配置方法让用户配置，或者让用户把KEY发给你自定配置到env文件中。

> **注意**：使用临时凭证（AKTP 开头）时，可以只使用 AK + Token，不需要 SK。

| 变量名                | 是否必需 | 说明                                           |
| --------------------- | -------- | ---------------------------------------------- |
| `OPENCLAW_MEDIA_PATH` | **必需** | OpenClaw media 目录绝对路径，图片/视频将保存到该目录下的 jimeng-generate 文件夹 |
...
优先使用`pnpm i `进行依赖安装，如果`pnpm`不存在，则使用`npm i `
...
```json
{
    ...
    "count": 1,
    "taskId": "1234567890",
    "images": ["./output/<md5_hash>/1.jpg", "./output/<md5_hash>/2.jpg"],
...
视频文件会自动下载到对应的任务文件夹中，文件名为 `video.mp4`。
