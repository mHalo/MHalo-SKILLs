 ...
 * 多维表格编辑者角色设置
 * 
 * 场景描述: 为指定的多维表格创建编辑者角色，并将指定用户添加为该角色的协作者
 * 
 * 使用的基础能力:
...
为指定的多维表格创建编辑者角色，并将指定用户添加为该角色的协作者。
编辑者拥有表格的编辑权限（table_perm = 2），可以查看和编辑数据。

用法: npx ts-node scripts/bitable-editor-role.ts [选项]
...
async function main() {
  ...
  const roleName = (options['role-name'] as string) || (options.r as string) || '编辑者';
  ...
  try {
    ...
    console.log(`角色名称: ${roleName}`);
    console.log(`用户 ID: ${userId}`);
    console.log(`权限级别: 可编辑 (table_perm = 2)`);
    console.log('='.repeat(60) + '\n');
